...
def main() :
    ...
    # 1D 
    tot = sum(val[:1]) / p
    print "Pourcentage de variance totale : {0}".format(tot)
    Y_1 = np.dot(X_, U[:,:1])
    ...
    # Erreur de reconstruction a partir de la matrice reduite a 1D  
    err = np.mean((np.absolute(X_ - np.dot(Y_1, U[:,:1].T))))
    ...
    # Erreur de reconstruction a partir de la matrice reduite a 2D  
    err = np.mean((np.absolute(X_ - np.dot(Y_2, U[:,:2].T))))
    print " Erreur absolue moyenne : %f"%err
    ...
    # Erreur de reconstruction a partir de la matrice reduite a 3D
    err = np.mean((np.absolute(X_ - np.dot(Y_3, U[:,:3].T))))
    print " Erreur absolue moyenne : %f"%err
    return 
